...
class SCFRecorder:
    ...
    def __call__(self, envs):
        # modified from: pyscf.scf.chkfile.dump_scf
        scf_dict = {
            'e_tot': envs['e_tot'],
            'mo_energy': envs['mo_energy'],
            'mo_occ': envs['mo_occ'],
            'mo_coeff': envs['mo_coeff'],
            'scf_summary': envs['mf'].scf_summary,
            'fock': envs['diis_fock'],
            'side_fock': envs['fock'],
        }
        self.states.append(copy.deepcopy(scf_dict))

    def save_to_npz(self, out_path, mol=None, name=None):
        out = {}
        if mol is not None:
            out['mol'] = mol.dumps()
        if name is not None:
            out['name'] = name
        for step, scf_dict in enumerate(self.states):
            for key, value in scf_dict.items():
                out[f'scf_{step}_{key}'] = value
        out['max_step'] = len(self.states)
...
Keyword argument "init_dm" is replaced by "dm0"''')
    ...
    for cycle in range(mf.max_cycle):
        ...
        diis_fock = mf.get_fock(h1e, s1e, vhf, dm, cycle, mf_diis) # lea: modified here
        mo_energy, mo_coeff = mf.eig(diis_fock, s1e)
        mo_occ = mf.get_occ(mo_energy, mo_coeff)
...
"""
"mf.kernel" aliases to this function, which in turn calls the "kernel" function.
* No code is modified, but the "kernel" this function calls now refers to our version above.
"""
